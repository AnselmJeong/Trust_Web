...
class ResultsState(rx.State):
    ...
    async def load_experiment_data(self, game_name: str, section_no: int = 1):
        ...
        try:
            ...
            if auth_state and hasattr(auth_state, 'is_authenticated') and auth_state.is_authenticated and hasattr(auth_state, 'user_id') and auth_state.user_id:
                ...
                self.statistics = get_user_experiment_data(current_user_id, game_name, section_no) 
                if not self.statistics:
                    print(f"[ResultState] No experiment data found for user: {current_user_id}, game: {game_name}")
                    self.statistics = [] 
                else:
                    print(f"[ResultState] Statistics loaded for game {game_name}: {len(self.statistics)} items")
            else:
            ...
            print(f"[ResultState] Error in load_experiment_data for game {game_name}: {e}")
    ...
    def pgg_overall_summary(self) -> dict:
        ...
        if isinstance(self.statistics[0], dict) and ("error" in self.statistics[0] or "error_loading" in self.statistics[0]):
            return default_summary
            ...
            if isinstance(item, dict) and "data" in item:
                ...
                if data.get("game_name") == "public goods game":
                    try:
                        ...
                        if data.get("round") is not None and isinstance(contribution, (int, float)) and isinstance(payoff, (int, float)):
                            contributions.append(contribution)
                            payoffs.append(payoff)
                            num_rounds += 1
                        else:
                            print(f"[PGG Overall Summary DEBUG] Skipping item due to missing/non-numeric round/contribution/payoff: {data}") 
                    except (KeyError, TypeError) as e:
                        print(f"[PGG Overall Summary DEBUG] Error processing item {data}: {e}")
                        continue
                else:
                    print(f"[PGG Overall Summary DEBUG] Skipping item, game_name mismatch. Expected: 'public goods game', Got: '{data.get('game_name')}'. Data: {data}")
            ...
            if isinstance(item, dict) and "data" in item:
                ...
                try:
                    if data.get("game_name") == "public goods game":
                        ...
                        if not all(isinstance(val, (int, float)) for val in [round_num, contribution, payoff]):
                            print(f"[ResultState-PGG_Round_Summary DEBUG] Skipping item with non-numeric data: {item}")
                            continue
                    ...
                    print(f"[ResultState-PGG_Round_Summary DEBUG] KeyError processing item {item}: {e}")
                except TypeError as e:
                    print(f"[ResultState-PGG_Round_Summary DEBUG] TypeError processing item {item} (likely non-numeric data): {e}")      
    ...
    def tg_section1_summary(self) -> dict:
        ...
        default_tg_summary = {
            "total_rounds": 0, 
            "total_amount_sent": 0, "avg_amount_sent": 0,
            "total_amount_returned": 0, "avg_amount_returned": 0,
            "player_a_balance": 0, "user_balance": 0,
            "player_a_payoff": 0, "user_payoff": 0
        }
        if self.current_game_loaded != "trust_game" or not self.statistics:
            return default_tg_summary
        if isinstance(self.statistics[0], dict) and ("error" in self.statistics[0] or "error_loading" in self.statistics[0] or "error_fetching" in self.statistics[0]):
            return default_tg_summary
        ...
        for item in self.statistics:
            if isinstance(item, dict) and "data" in item:
                data = item.get("data", {})
                if data.get("game_name") == "trust_game" and data.get("section_num") == 1:
                    total_rounds += 1
        ...
        avg_amount_sent = total_amount_sent / len(list_amount_sent) if list_amount_sent else 0
        total_amount_returned = sum(list_amount_returned)
        avg_amount_returned = total_amount_returned / len(list_amount_returned) if list_amount_returned else 0
        total_player_a_payoff = sum(list_player_a_payoff)
        avg_player_a_payoff = total_player_a_payoff / len(list_player_a_payoff) if list_player_a_payoff else 0
        total_player_b_payoff = sum(list_player_b_payoff)
        avg_player_b_payoff = total_player_b_payoff / len(list_player_b_payoff) if list_player_b_payoff else 0
        ...
        summary = {
            ...
            "player_a_balance": round(total_player_a_payoff, 2),
            "user_balance": round(total_player_b_payoff, 2),
